...
bool kbase_pm_clock_off(struct kbase_device *kbdev, bool is_suspend)
{
	...
	/* MALI_SEC_INTEGRATION */
	{
		u32 loops = KBASE_TRANS_READY_LOOPS;
		u32 raw;
		kbase_os_reg_write(kbdev, GPU_CONTROL_REG(L2_PWROFF_LO), 0xFFFFFFFF);
		raw = kbase_os_reg_read(kbdev, GPU_CONTROL_REG(L2_PWRTRANS_LO)) | kbase_os_reg_read(kbdev, GPU_CONTROL_REG(L2_READY_LO));
		while (raw == 1 && --loops) {
			raw = kbase_os_reg_read(kbdev, GPU_CONTROL_REG(L2_PWRTRANS_LO)) | kbase_os_reg_read(kbdev, GPU_CONTROL_REG(L2_READY_LO));
		}
		if (loops == 0) {
			dev_err(kbdev->dev, "GPU L2 off fail!(%d, %d)\n", raw, loops);
			spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
			return false;
		}
	}
